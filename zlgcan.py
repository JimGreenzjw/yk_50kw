...
'''
def can_E_U_start(zcanlib, device_handle, chn):
    ret = zcanlib.ZCAN_SetValue(device_handle, str(chn) + "/baud_rate", "250000".encode("utf-8")) #250Kbps
...
###SET filter  
    ...
    if ret != ZCAN_STATUS_OK:
        print("Set CH%d CAN_E_U filter_ack failed!" %(chn)) 
        exit(0)
...
if __name__ == "__main__":
    ...
    if handle == INVALID_DEVICE_HANDLE:
        print("Open USBCAN-XE-U device failed!")
        exit(0)
    ...
    chn_handle = can_E_U_start(zcanlib, handle, 0)
